...
// Optimized dynamic import with proper error boundary
const TestimonialsSection = dynamic(() => import('../components/TestimonialsSection'), {
  ssr: false,
  loading: () => <div className="h-64 bg-gray-200 animate-pulse" aria-hidden="true" />,
});

// Memoized components with proper dependency optimization
const ContentSection = memo(({
  ...
  // Memoized support items
  const supportItemElements = useMemo(() => 
    supportItems?.map((item, idx: number) => (
      <motion.div
        key={idx}
...
const HeroSectionComponent = memo(() => {
  ...
  useEffect(() => {
    ...
    checkIsMobile();
    const resizeHandler = () => checkIsMobile();
        ...
        <div className="absolute inset-0 z-0">
          {isImageLoaded ? (
            <AnimatePresence initial={false} mode="wait">
              ...
              >
                <Image
                  src={isMobile ? currentSlideData.mobileImage : currentSlideData.desktopImage}
                  alt={`${currentSlideData.title} in Nairobi, Kenya`}
                  fill
                  sizes="100vw"
                  className="object-cover"
                  ...
                  onError={() => console.error(`Failed to load image: ${isMobile ? currentSlideData.mobileImage : currentSlideData.desktopImage}`)}
                />
              </motion.div>
            </AnimatePresence>
          ) : (
                ...
                >
                  {isMobile ? (
                    <>
                      ...
                      <p className="text-white text-sm md:text-base mb-4 leading-relaxed">{currentSlideData.description}</p>
                      <motion.button
                        onClick={() => openProductModal(currentSlideData.title.replace(' Car Parts', ''))}
                        whileHover={{ scale: 1.05, boxShadow: '0 0 8px rgba(220, 38, 38, 0.5)' }}
                        whileTap={{ scale: 0.95 }}
                        ...
                        aria-label={currentSlideData.cta}
                      >
                    ...
                    <>
                      ...
                      <p className="text-white text-base mb-6 leading-relaxed max-w-lg">{currentSlideData.description}</p>
...
export default function LandingPage() {
  ...
  const productSections = useMemo(() => 
    products.map((product, index) => (
      ...
      />
    )), [products, openProductModal, openContactModal]
  );
