...
const ContentSection = memo(({
  ...
  return (
    <section className="py-8 md:py-12 bg-white overflow-hidden" role="region" aria-labelledby={`section-${title.replace(/\s+/g, '-')}`}>
      <div className="container mx-auto px-4 sm:px-6">
        <div className={`grid lg:grid-cols-2 gap-4 md:gap-6 items-center ${isReversed ? 'lg:grid-flow-col-dense' : ''}`}>
          <motion.div
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: "-50px" }}
            className={`${isReversed ? 'lg:col-start-1 lg:mr-[-30px]' : 'lg:ml-[-30px]'} relative z-20 mt-[-40px] md:mt-[-60px] w-full max-w-[95%] mx-auto lg:max-w-none h-[320px] sm:h-[380px] md:h-[420px] lg:h-auto lg:overflow-visible overflow-y-auto scrollbar-hide`}
          >
            <div className="bg-gray-100 p-4 md:p-6 lg:p-7 text-black h-full">
              <motion.div
                initial={{ width: 0 }}
              ...
              <div className="flex items-center space-x-2 md:space-x-3 mb-3 md:mb-4">
                ...
                <h2 id={`section-${title.replace(/\s+/g, '-')}`} className="text-2xl md:text-3xl lg:text-4xl font-serif font-bold text-black">
                  {title}
                </h2>
              </div>
                ...
                <div className="space-y-2 mb-4 md:mb-5">
                  {features.map((feature: string, idx: number) => (
                    ...
                    >
                      ...
                      <span className="text-black text-sm md:text-base">{feature}</span>
                    </motion.div>
                  ))}
                </div>
              )}
